...
async function main() {
    ...
    const pathProvider = new gltfModelPathProvider(
        "assets/ux3dModels/model-index.json"
    );
                ...
                resourceLoader
                    ...
                    .then((gltf) => {
                        ...
                        if (state.gltf.scenes.length != 0) {
                            ...
                            state.userCamera.perspective.aspectRatio.fallbackValue =
    ...
    const sceneChangedStateObservable = uiModel.scene.pipe(
        map((newSceneIndex) => state)
    );
    ...
    const update = () => {
        ...
        redraw |=
            !state.animations.every((animation) =>
                animation.timer.isPaused()
            ) && state.animationIndices.length > 0;
        redraw |= past.width != canvas.width || past.height != canvas.height;
        redraw |=
            state.gltf &&
            state.gltf.behaviors &&
            state.gltf.behaviors.length > 0;
